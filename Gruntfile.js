...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Ben Hell';
    var email     = grunt.option('email')  || 'ben@oject-d.net';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new solvis name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/solvis.png')) {
            fs.renameSync(__dirname + '/admin/solvis.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/solvis.html')) {
            fs.renameSync(__dirname + '/widgets/solvis.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/solvis/js/solvis.js')) {
            fs.renameSync(__dirname + '/widgets/solvis/js/solvis.js', __dirname + '/widgets/solvis/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/solvis')) {
            fs.renameSync(__dirname + '/widgets/solvis',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Solvis/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Solvis'
                        },
                        {
                            match: /Ben Hell/g,
                            replacement: author
